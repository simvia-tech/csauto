...
- A code_saturne case launched on Qarnot failed in preprocessing with `Mesh file ... not found (no mesh directory given)`. The case inputs were uploaded at the root of the remote working directory, so the campaign's shared directories landed *inside* the case instead of beside it, while code_saturne looks for its mesh in `<study>/MESH`. The remote layout now mirrors the local one, with the case in a directory of its own; the results a task returns are restricted to that directory, so a download can no longer reach `RUNS/MESH`, which is a symlink to the user's own mesh
